app.controller('ingresoExaGraciaCtrl', function(Excel, $timeout,$scope, $http) {
...
//////////////////////////////////////////////////////////////////////////
	...
	function buscarIdCurso(nombreCurs){
		...
		$http({
			...
			$scope.getUrl = $('#urlNumRegistrosGracia').val();
...
/////////////////////////////////////////////////////////////////////////
	...
	$scope.enviarDatosExa = function(){
		...
		for(var i = 0; i < notas.length; i+=2){
			...
			*/
			...
			$scope.getUrl = $('#urlIngresarGracia').val();
	...
	function consultarNotasFinalesMejora(cedula){
		var url = $('#urlNotasTotalesGracia').val();
		$http({
            method: "post",
            url: url,
            data:   "cursoId="+$scope.cursoId
...
////////////////////////////////////////////////////////////////////////
	...
	$scope.mostrarNotasGraciaEditar = function(event){
		var idSuple = event.target.id;
		
		$scope.getUrl = $('#urlNotasGraciaEdit').val();
	...
	function consultarNotasMejora(url, id){
		...
		$http.get(urlSuple).success(function(response){
			...
			$scope.idSuple 		= 	response[0]['id_gra'];
			$scope.notaSuple 	= 	response[0]['nota_gra'];
        }, function (error) {
                console.log(error);
        });
	}
	...
	$scope.procesoActualizar = function(){
		var idEdit = $('#idGraciaEdit').val();

		$scope.getUrl = $('#urlActualizarGracia').val();
